...
u_csrbf = results(X, Y, nx, ny, s, U, max_span_x = max_span_x, max_span_y = max_span_y, span = span, r_xy = r_xy, support = support)
...
print("npoints = {} scaling parameter = {} max_span = {} Least square error ={}".format( nx*ny, s, max(max_span_x,max_span_y), np.sqrt(np.sum(( u_csrbf - u_exact)**2)) ) )
...
#axes[0].axis('off')
axes[0].margins(0,0)
im    = axes[1].contourf( X, Y, u_csrbf, cmap= 'jet')
...
surf0 = ax.plot_surface(X, Y, u_exact, rstride=1, cstride=1, cmap=cm.coolwarm,
...
surf = ax.plot_surface(X, Y, u_csrbf, cmap=cm.coolwarm,
                       linewidth=0, antialiased=False)
#ax.set_xlim(0.0, 1.0)
#ax.set_ylim(0.0, 1.0)
ax.zaxis.set_major_locator(LinearLocator(10))
