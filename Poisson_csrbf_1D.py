...
from simple_CSRBF                   import CSRBF_basis_1D
from simple_CSRBF                   import results_1D
...
U_CSRBF_ET = results_1D(X, nx, s, U, max_span = max_span, span = span, r_x = r_x, support = support)
...
print("scaling parameter = {} max_span = {} Least square error ={}".format( s, max_span, np.sqrt(np.sum(( U_CSRBF_ET - U_exact_ET)**2)) ) )

##  plot
plt.figure()
plt.axes().set_aspect('equal')
plt.plot( X, U_CSRBF_ET,  color='r', lw = 2.5, ls='-', marker='s', markersize=1, markerfacecolor = "k", label='$\mathbf{CSRBF}$')
plt.plot( X, U_exact_ET, color='b', lw = 2.5, ls='-', marker='v', markersize=1, markerfacecolor = "k", label = '$\mathbf{EXACT}$')
